 ...
 *
 * This program essentially implements a concurrent proxy through processes.
 * In order to handle multiple clients simultaneously, we use processes to 
 * distinguish through each of the child processes everytime a new client communicates
 * with the server. It also locks the logging so that all of the processes don't keep writing
 * to the file.
 */
